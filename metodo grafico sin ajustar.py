...
class ventana:
    def __init__(self):
        ...
        self.CBMax_Min = tkinter.ttk.Combobox(self.frame1,textvariable=self.ValorMax_Min,width="10",font=("Helvetica", 10))
    ...
    def AgregarFO(self,fo,max_min,label):
        if fo.get()=="":
            tkinter.tkMessageBox.showinfo("ERROR", "No se ha ingresado función objetivo")
        else:
            foCopia=fo.get()
            try:
                ...
                tkinter.tkMessageBox.showinfo("ERROR", "Error al ingresar los datos")
    ...
    def insertarListbox(self,listbox,rest):
        if rest.get()=="":
            tkinter.tkMessageBox.showinfo("ERROR", "No ha ingresado restricciones")
        else:
            try:
                ecuacion=rest.get()
                if ecuacion.find("<=")>1:
